...
export class Lazy<T> implements LazyLike<T> {
    ...
    each<T>(this: Lazy<T>, handler: (value: PulledAwaited<T>) => Lazy<any>): Lazy<T>
    each<T>(this: Lazy<T>, handler: (value: PulledAwaited<T>) => any): Lazy<T>

    /**
     * Returns a new Lazy, **WAITING**, that will execute **THIS** and invoke the given handler on
     * the result. It will wait for the handler to finish before producing the same value.
     *
