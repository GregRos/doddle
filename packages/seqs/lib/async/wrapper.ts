...
export class ASeq<E> {
    ...
    count(fn?: AsyncPredicate<E>): LazyAsync<number> {
        return this.reduce(
            async (acc, item, i) => ((fn ? await fn.call(this, item, i) : true) ? acc + 1 : acc),
            0
        )
    }
    ...
    filterAs<X, R extends X>(fn: (this: ASeq<E>, item: X, index: number) => item is R): ASeq<R> {
    ...
    zip<Xs extends any[]>(
        ...
        return this._wrap(async function* (self) {
            const iterators = [self, ...others].map(x => x[Symbol.asyncIterator]())
            for (;;) {
                const results = await Promise.all(iterators.map(async iterator => iterator.next()))
