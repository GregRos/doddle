...
declare.it("accepts an input sequence, returns Doddle<boolean>", expect => {
    ...
    expect(type_of(s.seqEqualsBy(s))).to_equal(type<Doddle<boolean>>)
})
declare.it("accepts input sequence of subtype, returns Doddle<boolean>", expect => {
    ...
    expect(type_of(s1.seqEqualsBy(s2))).to_equal(type<Doddle<boolean>>)
})
...
it("returns true for empty sequences", () => {
    const s = _seq([]).seqEqualsBy(_seq([]))
    expect(s.pull()).toEqual(true)
})
it("returns false for empty vs singleton", () => {
    const s = _seq([]).seqEqualsBy(_seq([1]))
    expect(s.pull()).toEqual(false)
})

it("returns true for same sequence", () => {
    const s = _seq([1, 2, 3]).seqEqualsBy(_seq([1, 2, 3]))
    expect(s.pull()).toEqual(true)
})

it("returns false for different sequences", () => {
    const s = _seq([1, 2, 3]).seqEqualsBy(_seq([1, 2, 4]))
    expect(s.pull()).toEqual(false)
})

it("returns false for different lengths", () => {
    const s = _seq([1, 2, 3]).seqEqualsBy(_seq([1, 2]))
    expect(s.pull()).toEqual(false)
})
...
it("accepts different seq input types", () => {
    ...
    expect(
        s1
            .seqEqualsBy(function* () {
                yield 1
                yield 2
            })
            .pull()
    ).toEqual(true)
    expect(s1.seqEqualsBy([1, 2]).pull()).toEqual(true)
    expect(s1.seqEqualsBy(() => s1[Symbol.iterator]()).pull()).toEqual(true)
})
