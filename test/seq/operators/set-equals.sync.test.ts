...
declare.it("accepts an input sequence, returns Doddle<boolean>", expect => {
    ...
    expect(type_of(s.setEqualsBy(s))).to_equal(type<Doddle<boolean>>)
})
declare.it("accepts input sequence of subtype, returns Doddle<boolean>", expect => {
    ...
    expect(type_of(s1.setEqualsBy(s2))).to_equal(type<Doddle<boolean>>)
})
...
it("accepts different seq input types", () => {
    ...
    expect(
        s1
            .setEqualsBy(function* () {
                yield 1
                yield 2
            })
            .pull()
    ).toEqual(true)
    expect(s1.setEqualsBy([1, 2]).pull()).toEqual(true)
    expect(s1.setEqualsBy(() => s1[Symbol.iterator]()).pull()).toEqual(true)
})
