...
it("returns singleton on singleton", () => {
    const s = _seq([1]).uniq(() => 1)
    expect(s._qr).toEqual([1])
})

it("removes duplicates", () => {
    const s = _seq([1, 2, 1, 2]).uniq(() => 1)
    expect(s._qr).toEqual([1])
})

it("removes duplicates with weird elements, also maintains order", () => {
    const s = _seq([1, NaN, -0, undefined, undefined, 0, null, 1, undefined]).uniq(x => x)
...
it("works with doddle iteratee", () => {
    const s = _seq([1, 2, 1, 2]).uniq(x => doddle(() => x))
