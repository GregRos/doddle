...
declare.it("is typed as Seq of key-value tuples", expect => {
    const source = { a: 1, b: 2 } as const
    expect(type_of(_seq.fromObject(source))).to_equal(type<_Seq<["a" | "b", 1 | 2]>>)
})

it("works on empty input", () => {
    expect(_seq.fromObject({})._qr).toEqual([])
})

it("works on single key input", () => {
    expect(_seq.fromObject({ a: 1 })._qr).toEqual([["a", 1]])
})

it("works on larger object", () => {
    expect(_seq.fromObject({ a: 1, b: 2, c: 3 })._qr).toEqual([
        ["a", 1],
        ["b", 2],
        ["c", 3]
    ])
...
it("does not produce symbol keys", () => {
    const sym = Symbol("hidden")
    const source: any = { [sym]: 1, a: 2 }
    expect(_seq.fromObject(source)._qr).toEqual([["a", 2]])
})

it("does not produce non-enumerable kvps", () => {
    const source: any = {}
    Object.defineProperty(source, "hidden", { value: 1, enumerable: false })
    Object.defineProperty(source, "visible", { value: 2, enumerable: true })
    expect(_seq.fromObject(source)._qr).toEqual([["visible", 2]])
})

it("does not produce inherited kvps", () => {
    const base = { inherited: 1 }
    const source = Object.create(base)
    source.own = 2
    expect(_seq.fromObject(source)._qr).toEqual([["own", 2]])
})

it("values are produced lazily", () => {
...
it("array input produces numeric string keys", () => {
    expect(_seq.fromObject(["a", "b"])._qr).toEqual([
        ["0", "a"],
        ["1", "b"]
    ])
})
